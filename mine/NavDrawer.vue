...
onMounted(async () => {
  try {
    const drawer01Response = await fetch('/json/Drawer01.json')
    const drawer01Data = await drawer01Response.json()
    drawer01Items.value = drawer01Data

    const drawer02Response = await fetch('/json/Drawer02.json')
    const drawer02Data = await drawer02Response.json()
    drawer02Items.value = drawer02Data
  } catch (error) {
